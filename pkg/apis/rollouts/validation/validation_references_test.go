...
`
	...
	for _, routeType := range routeTypes {
		t.Run("will succeed with valid appmesh virtual-router with httpRoute", func(t *testing.T) {
			t.Parallel()
...
`
		...
		assert.NotNil(t, errList)
		assert.Len(t, errList, 1)
		assert.Equal(t, errList[0].Field, field.NewPath("spec", "routes").Index(0).Child("httpRoute").Child("action").Child("weightedTargets").String())
	})
